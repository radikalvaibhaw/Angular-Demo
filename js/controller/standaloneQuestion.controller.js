...
app.controller("standaloneQuestionCtrl",['$scope','$routeParams','standaloneQuestionService','loginService','urls','$http','Upload','$timeout','$location','standaloneService',
	function($scope,$routeParams,standaloneQuestionService,loginService,urls,$http,Upload,$timeout,$location,standaloneService){
		...
		var getList = function(){
			...
			standaloneQuestionService.getList($routeParams.id,function(res){
				$scope.results = res;
		...
		{
			window.location.href="#!/standalone/question/"+level_id;
		}
		...
		$scope.deleteTournament = function(id){
			...
			if(temp){
				$scope.isBusyButton = true;
				standaloneQuestionService.deleteMethod(id,function(res){
					console.log(res);
					$scope.message = res;
					if(res){
						getList();
		...
		$scope.openEditModal = function(questionId){
			...
			$scope.isBusyButton = true;
			standaloneQuestionService.getDetails(questionId,function(res){
				$scope.isBusyButton = false;
				$scope.formData = res;
			});
		};
